...
#TODO : Check construction is resilient to weird image sizes
...
class DiamondSquareMap(HeightMap):
    ...
    def build_mapping(self):
        bx = 1
        while (3 * bx + 1) < self.image.width:
            bx = bx * 2
        totalsize = bx + 1

        boxmap = self._get_box((3 * bx) + 1, by + 1)

        self.setup(boxmap, int(bx / 2), 0)
        self.setup(boxmap, int(bx / 2), bx)
        self.setup(boxmap, int(3 * bx / 2), 0)
        self.setup(boxmap, int(3 * bx / 2), bx)
        self.setup(boxmap, int(5 * bx / 2), 0)
        self.setup(boxmap, int(5 * bx / 2), bx)
